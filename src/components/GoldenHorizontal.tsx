...
export default function GoldenHorizontal({
    ...
    useEffect(() => {
        const observer = new ResizeObserver(() => {
            if (!parent.current || !topElement.current || !bottomElement.current) return;

            const ratio = parent.current.offsetHeight / 2.618;

            const topPos = ratio - topElement.current.getBoundingClientRect().height / 2;
            topElement.current.style.top = `${topPos}px`;

            const bottomPos = ratio - bottomElement.current.getBoundingClientRect().height / 2;
